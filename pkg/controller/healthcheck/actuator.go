...
/*
	...
	Each Actuator is responsible for a single extension resource (e.g Worker) - predicates can be defined for fine-grained control over which objects to watch.
...
// HealthActuator acts upon registered resources.
type HealthCheckActuator interface {
	...
	// Returns ExtensionHealthCheckResult for each healthConditionType registered with the individual health checks.
	...
	ExecuteHealthCheckFunctions(context.Context, types.NamespacedName) (*[]ExtensionHealthCheckResult, error)
}

// ExtensionHealthCheckResult represents an aggregated health status for the health checks performed on the dependent API Objects of an extension resource.
// An ExtensionHealthCheckResult refers to a single healthConditionType (e.g SystemComponentsHealthy) of an extension Resource.
...
func (h *ExtensionHealthCheckResult) GetDetails() string {
	if h.Detail == nil {
		return ""
	}
	return *h.Detail
...
// HealthCheckResult is the result for a health check
