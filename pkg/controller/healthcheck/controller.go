...
// DefaultRegisterExtensionForHealthCheck configures the default health check Actuator to execute the provided health checks and adds it to the provided controller-runtime manager.
// the Actuator reconciles a single extension with a specific type and writes a Conditions for each distinct healthConditionType.
...
func DefaultRegisterExtensionForHealthCheck(extensionType string, kind schema.GroupVersionKind, register RegisterExtension, mgr manager.Manager, opts DefaultAddArgs, customPredicates []predicate.Predicate, healthChecks map[HealthCheck]string) error {
	...
	healthCheckActuator := Actuator(args.Type, args.GetExtensionGroupVersionKind().Kind, healthChecks)
...
func add(mgr manager.Manager, args AddArgs) error {
	...
	log.Log.Info("Registered health check controller", "Kind", args.registeredExtension.groupVersionKind.Kind, "type", args.Type, "health check type", args.registeredExtension.healthConditionType, "sync period", args.SyncPeriod.Duration.Round(0).String())

	if err := ctrl.Watch(&source.Kind{Type: args.registeredExtension.register()}, &handler.EnqueueRequestForObject{}, predicates...); err != nil {
		return err
	}
	return nil
}
