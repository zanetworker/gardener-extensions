...
func (a *actuator) cloudConfigFromOperatingSystemConfig(ctx context.Context, config *extensionsv1alpha1.OperatingSystemConfig) ([]byte, error) {
	files := make([]*File, 0, len(config.Spec.Files))
	for _, file := range config.Spec.Files {
		...
		files = append(files, &File{Path: file.Path, Content: data, Permissions: file.Permissions})
	}

	units := make([]*Unit, 0, len(config.Spec.Units))
	for _, unit := range config.Spec.Units {
		...
		dropIns := make([]*DropIn, 0, len(unit.DropIns))
		for _, dropIn := range unit.DropIns {
			dropIns = append(dropIns, &DropIn{Name: dropIn.Name, Content: []byte(dropIn.Content)})
		}
		units = append(units, &Unit{Name: unit.Name, Content: content, DropIns: dropIns})
	}

	return NewCloudInitGenerator(DefaultUnitsPath).Generate(&OperatingSystemConfig{Files: files, Units: units})
}
