...
func (e *ensurer) EnsureKubeletCloudProviderConfig(ctx context.Context, data *string, namespace string) error {
	...
	err := e.client.Get(ctx, kutil.Key(namespace, azure.CloudProviderWorkerConfigName), &cm)
	if err != nil {
		if apierrors.IsNotFound(err) {
			e.logger.Info("configmap not found", "name", azure.CloudProviderWorkerConfigName, "namespace", namespace)
			return nil
		}
		return errors.Wrapf(err, "could not get configmap '%s/%s'", namespace, azure.CloudProviderWorkerConfigName)
	}
