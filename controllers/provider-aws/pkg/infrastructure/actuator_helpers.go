...
func (c *actuator) injectProviderStateIntoStatus(ctx context.Context, tf *terraformer.Terraformer, infrastructure *extensionsv1alpha1.Infrastructure) error {
	values, err := tf.GetStateOutputVariables(awstypes.VPCIDKey, awstypes.SecurityGroupsNodes, awstypes.SSHKeyName, awstypes.IAMInstanceProfileNodes, awstypes.SubnetNodes, awstypes.SubnetPublic, awstypes.NodesRole)
	if err != nil {
		return err
	}
